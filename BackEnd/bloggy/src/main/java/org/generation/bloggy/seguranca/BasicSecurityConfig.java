...
/**
 * Características:
 *     - Filha (extende) a classe externa WebSecurityConfigurerAdapter
 *     - Injeta a classe vizinha(mesmo pacote) UserDetailsServiceImpl(q por sua vez) 
 *     - Sobrescreve o método configure, que recebe um obj autenticationManagerBuilder
 *     e acessa o método userDetailsService para registrar e criar uma nova instancia
 *     de userDetailsService.
 *     - cria uma @Bean, que é simplesmente "um autowired que pode ser usado em qqer lugar"
 *     
 */
 ...
 * 
 * Para personalizar a autenticação utilizaremos a sobrescrita dos métodos da
 * Classe WebSecurityConfigurerAdapter(Extendida por (pai de) nossa classe BasicSecurityConfig)
 * 
 */
...
public class BasicSecurityConfig extends WebSecurityConfigurerAdapter{
	 ...
	 */
	@Autowired
	private UserDetailsServiceImpl userDetailsService;  //Aqui o certo é a Impl ou a standard??
	 ...
	 *  UserDetailsService.
	 *  Outra alternativa de login seria acriação de um usuário em memória.
	 *  
	 *  O método é do tipo protected por definição da classe.
	 */

	
	@Override      //Entender pq usar o trhows aqui
	...
	protected void configure(HttpSecurity http) throws Exception{
		...
		.and().httpBasic()
		.and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS) // Estudar
		.and().cors()
		.and().csrf().disable();
	}
